    ...
    return (
        <div className='md:top-45 left-0 relative md:sticky flex md:flex-row flex-col-reverse gap-2 py-6 h-fit'>
            <div className='flex flex-row md:flex-col flex-nowrap gap-2 overflow-auto'>
                {images.map((image, i) => (
                    <div key={image + i} className={images.length === 1 ? "h-20 rounded-md overflow-hidden" : `rounded-md shrink-0 w-20 h-23 overflow-hidden ${mainImage === image && "border-2 border-[#E41F66]"} `}>
